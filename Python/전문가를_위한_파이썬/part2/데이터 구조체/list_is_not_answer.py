...
# 2.11 읽을 거리
## 혼합된 자료형 리스트
### 리스트는 여러 자료형의 객체를 담을 수 있다고 강조하지만, 사실 이 기능은 그리 쓸모 있지 않다.
l = [28, 14, '28', 5, '23']
sorted(l)  # TypeError: underable
### 리스트와 달리 튜플은 다른 자료형을 담는 경우가 종종 있다.

## key 는 훌륭하다.
### list.sort() sorted(), max(), min() 함수의 선택적 인수인 `key`는 멋진 개념이다.
### key는 객체들을 정렬하기 위해 사용할 기준을 가져오거나 계산하는 인수 하나를 받는 함수를 정의하면 되므로 더 간단하다.
###* key를 사용하면 숫자와 숫자 형태의 문자열로 구성된 리스트도 정렬할 수 있다.
l = [28, '28', 1, '1', '3', 6]
sorted(l, key=int)  # [1, '1', '3', 6, 28, '28']
sorted(l, key=str)  # [1, '1', 28, '28', '3', 6] str의 경우 앞의 숫자를 본다. '`2`8'

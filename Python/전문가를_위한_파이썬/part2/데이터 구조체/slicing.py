...
# 불변 시퀀스에 반복적으로 연결 연산을 수행하는 것은 비효율적이다. 튜플 = 불변성

## 2.6.1 += 복합 할당 퀴즈
tup = (1, 2, [30, 40])
tup[2] += [50, 60]  # type error
# tup[2].extend([50, 60])  # 요소를 추가한다. += 연산자의 기괴한 작동 과정
## 1. 가변 항목을 튜플(불변)에 넣는 것은 좋은 생각이 아니다.
## 2. 복합 할당(+=, *=) 은 원자적인 연산이 아니다.
## 3. 바이트코드 살펴보기
### import dis
### dis.dis('s[a] += b')
